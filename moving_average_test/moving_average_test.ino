...
float movingAverage(float newSample) {
  ma_buffer[ma_index] = newSample;
  ma_index = (ma_index + 1) % MA_WINDOW;
  
  if (ma_index == 0) ma_filled = true;
  
  float sum = 0;
  int count = ma_filled ? MA_WINDOW : ma_index;
...
void calculateAndDisplayStats() {
  Serial.println("\n\n========================================");
  Serial.println("  MOVING AVERAGE TEST - FINAL RESULTS");
  Serial.println("  Filter: Moving Average (N=7)");
  Serial.println("========================================\n");
...
void setup() {
  ...
  Serial.println("\n=== MOVING AVERAGE TEST STARTING ===");
  Serial.println("Filter: N=7 samples\n");
...
void loop() {
  ...
  if (measure.RangeStatus != 4) {
    ...
    float dist_filtered = movingAverage((float)dist_raw);
